...
class MarvelmindHedge(Thread):
    def __init__(self, tty="/dev/ttyACM0", baud=9600, maxvaluescount=8, debug=False, recieveDataCallback=None):
        ...
        for x in range(0, 10):
            self.lastValues.append([0,
                                    0,
                                    0,
                                    0,
                                    0])  # last measured positions and timestamps; [adr, x, y, z, timestamp]
        self.pause = False
    ...
    def print_position(self):
        if isinstance(self.position()[1], int):
            print("Hedge {:d}: X: {:d}, Y: {:d}, Z: {:d} at time T: {:.2f}".format(self.position()[0],
                                                                                   self.position()[1],
                                                                                   self.position()[2],
                                                                                   self.position()[3],
        ...
        while not self.terminationRequired:
            if not self.pause:
                try:
                    ...
                    while (readChar is not None) and (readChar is not '') and (not self.terminationRequired):
                        ...
                        if 0 <= pktHdrOffset < 220 and len(bufferList) > (pktHdrOffset + 4):
                            ...
                            if pktHdrOffsetMm != -1:
                                ...
                                if self.debug:
                                    print ('Message with mm-data was detected')
                            elif pktHdrOffsetCm != -1:
                                isCmMessageDetected = True
                                if self.debug:
                                    print ('Message with cm-data was detected')
                            ...
                            # print 'len of buffer:
                            # ', len(bufferList), '; len of msg: ', msgLen, '; offset: ', pktHdrOffset
                            if len(bufferList) > pktHdrOffset + 4 + msgLen + 2:
                                usnCRC16 = 0
                                if isCmMessageDetected:
                                    usnTimestamp, usnX, usnY, usnZ, usnAdr, usnCRC16 = \
                                        struct.unpack_from('<LhhhxBxxxxH', strbuf, pktHdrOffset + 5)
                                elif isMmMessageDetected:
                                    usnTimestamp, usnX, usnY, usnZ, usnAdr, usnCRC16 = \
                                        struct.unpack_from('<LlllxBxxxxH', strbuf, pktHdrOffset + 5)
                                    usnX = usnX/10.0
                                    usnY = usnY/10.0
                                    usnZ = usnZ/10.0
                                ...
                                if pktHdrOffset == -1:
                                    if self.debug:
                                        print ('\n*** ERROR: Marvelmind USNAV beacon packet header not found '
                                               '(check modem board or radio link)')
                                    continue
                                elif pktHdrOffset >= 0:
                                    if self.debug:
                    ...
                    if self.debug:
                        print ('\n*** ERROR: serial port error (possibly beacon is reset, '
