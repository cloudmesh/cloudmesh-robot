...
<td><button name="LEFTANGLE" value="-90" type="submit">LEFTANGLE</button></td>
<td><button name="RIGHTANGLE" value="90" type="submit">RIGHTANGLE</button></td>
<td><button name="BACKWARD" value="1" type="submit">BACKWARD</button></td>
    ...
    if request[7] == '?':
        ...
        for param in params:
            ...
            if name == 'FORWARD':
                ...
                right.stop()
            if name == 'BACKWARD':
                left.backward(left.d)
                right.backward(right.d)
                time.sleep(value)
                left.stop()
                right.stop()
            if name == 'LEFT':
