...
def twos_comp(val, bits):
    ...
    class accelcomp:
        ...
        def __init__(self):
            ...
            if (whoami == 0x49):
                bus.write_byte_data(LSM303D_ADDR, CTRL_REG1,
                                    0x57)  # 0x57 = ODR=50hz, all accel axes on ## maybe 0x27 is Low Res?
                bus.write_byte_data(LSM303D_ADDR, CTRL_REG2, (3 & lt; & lt;
                6) | (0 & lt; & lt;
                3))  # set full scale +/- 2g
                bus.write_byte_data(LSM303D_ADDR, CTRL_REG3, 0x00)  # no interrupt
                bus.write_byte_data(LSM303D_ADDR, CTRL_REG4, 0x00)  # no interrupt
                bus.write_byte_data(LSM303D_ADDR, CTRL_REG5, (4 & lt; & lt;
                2))  # 0x10 = mag 50Hz output rate
        ...
        def getMag(self):
            self.mag[X] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_X_H_M) & lt; & lt;
            8 |
            bus.read_byte_data(LSM303D_ADDR, OUT_X_L_M), 16)
            self.mag[Y] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_Y_H_M) & lt; & lt;
            8 |
            bus.read_byte_data(LSM303D_ADDR, OUT_Y_L_M), 16)
            self.mag[Z] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_Z_H_M) & lt; & lt;
            8 |
            bus.read_byte_data(LSM303D_ADDR, OUT_Z_L_M), 16)

            def getAccel(self):
                accel = [0, 0, 0]
                accel[X] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_X_H_A) & lt; & lt;
                8 |
                bus.read_byte_data(LSM303D_ADDR, OUT_X_L_A), 16)
                accel[Y] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_Y_H_A) & lt; & lt;
                8 |
                bus.read_byte_data(LSM303D_ADDR, OUT_Y_L_A), 16)
                accel[Z] = twos_comp(bus.read_byte_data(LSM303D_ADDR, OUT_Z_H_A) & lt; & lt;
                8 |
            ...
            def getHeading(self):
                ...
                if self.heading & lt; 0:
                    self.heading += 2 * math.pi
                if self.heading & gt; 2 * math.pi:
                    self.heading -= 2 * math.pi
            ...
            def getTiltHeading(self):
                ...
                try:
                    ...
                    roll = math.asin(truncate[Y] / math.cos(pitch)) if abs(math.cos(pitch)) & gt;= abs(
                        truncate[Y]) else 0
                    ...
                    if self.tiltHeading & lt; 0:
                        self.tiltHeading += 2 * math.pi
                    if self.tiltHeading & gt; 2 * math.pi:
                        self.heading -= 2 * math.pi
                    ...
                    AccelX
                    {}, AccelY
                    {} & quot;.format(self.accel[X], self.accel[Y])
                    print & quot;
                    TruncX
                    {}, TruncY
                    {} & quot;.format(truncate[X], truncate[Y])
                    print & quot;
                    Pitch
                    {}, cos(pitch)
                    {}, Bool(cos(pitch))
                    {} & quot;.format(pitch, math.cos(pitch), bool(math.cos(pitch)))

            def isMagReady(self):
                temp = bus.read_byte_data(LSM303D_ADDR, STATUS_REG_M) & amp;
                0x03
        ...
        if __name__ == &  # 039;__main__&#039;:

        from time import sleep

        lsm = accelcomp()

        print &  # 039;     Accelerometer     ||        Compass        ||              Heading&#039;
        print &  # 039;   x   |   y   |   z   ||   x   |   y   |   z   ||       calc      | tilt corrected  |&#039;
        while True:
            lsm.getAccel()
            ...
            lsm.getHeading()
            lsm.getTiltHeading()
            print &  # 039;{0:.3f} |{1:.3f} |{2:.3f} ||{3:6} |{4:6} |{5:6} ||  {6:6} deg     |  {7:6} deg     |&#039;.format(
            lsm.accel[X], lsm.accel[Y], lsm.accel[Z], lsm.mag[X], lsm.mag[Y], lsm.mag[
                Z], lsm.headingDegrees, lsm.tiltHeadingDegrees)
            sleep(.5)